...
from local_utils import EncoderRnn, Attention, Encoder, Decoder, ARNN
...
arnn.load_state_dict(torch.load(path, map_location=torch.device('cpu')))
arnn.eval()
...
with torch.no_grad():
    logits = arnn(inputs)  # logits = torch.Size([1015, 6])
    ...
    logits = arnn(inputs)  # torch.Size([1015, 6])
    ...
    # predicted_std = np.std(methane_absolute_error, 0, ddof=1)
    # print(predicted_std)
    # standard_error = predicted_std / (np.sqrt(1015))
    ...
    # plt.yscale("log")
    # ax1.plot(methane_absolute_error, linestyle="--", marker="*", color="C0")
    # ax1.set_xlabel("sample index", color="C0")
    # ax1.set_ylabel("Absolute_error (ppm)", color="C0")
    ...
    plt.scatter(methane_label_concentration[257:], methane_predicted_concentration[257:], s=20, color="red",
    ...
    plt.scatter(actone_label_concentration[257:], actone_predicted_concentration[257:], s=20, color="red",
                edgecolors="black", alpha=0.5)
    ...
    plt.scatter(water_label_concentration[257:], water_predicted_concentration[257:], s=20, color="red",
                edgecolors="black", alpha=0.5)
